...
from loop.kernel import boot, rootfs
...
def init(args):
    ...
    try:
        ...
        # Assuming legacy plugins were in ~/.loop/plugins and we want them there?
        # rootfs.init_structure creates ~/.loop/plugins.
        # If they were somewhere else? The prompt doesn't specify legacy location for plugins.
        # Assuming they were in ~/.loop/plugins already or not relevant.

        # Execute Structure Creation
        rootfs.init_structure()

        print(f"LooP v0.7.1 initialized at {rootfs.LOOP_ROOT}")
...
def start(args):
    """
    Start the LooP Shell (Default).
    """
    print("Booting LooP Shell...")
    try:
        kernel = boot.boot()
...
def serve(args):
    ...
    print(f"Starting LooP Server on {args.host}:{port}...")
    print(f"[Server] Listening on port: {port}")
    print(f"PORT: {port}")
    try:
        import uvicorn
        # We run the app defined in loop.server.main
        uvicorn.run("loop.server.main:app", host=args.host, port=port, reload=False)
    except ImportError:
...
def check_rootfs_write():
    """Tries to write a temp file to ~/.loop/tmp. Returns True if successful."""
    try:
        test_path = Path.home() / ".loop" / "tmp" / "test_write"
...
def doctor(args):
    ...
    # 4. Sidecar Handshake
    sidecar_port = os.environ.get("LOOP_SIDECAR_PORT")
    if sidecar_port:
        print(f"[Sidecar]   LOOP_SIDECAR_PORT detected: {sidecar_port}")
    else:
        print("[Sidecar]   No Tauri environment detected (Standalone)")

def main():
    """Main entry point for the CLI script."""
    parser = argparse.ArgumentParser(description="LooP Command Line Interface")
