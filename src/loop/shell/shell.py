...
This module provides the command-line interface (CLI) for LooP users.
...
class Shell:
    ...
    def __init__(self, syscall, service_manager=None, io_adapter=None):
        ...
        """
        from loop.kernel.io import CLIAdapter
    ...
    def run(self):
        ...
        """
        while self.running:
            cmd = self._readline(f"{self.current_user}@loop:{self.cwd}> ")
    ...
    def _run_program(self, args):
        """
        Execute an external program located in `loop.bin`.
        ...
        try:
            mod = import_module(f"loop.bin.{program}")
        except ImportError:
