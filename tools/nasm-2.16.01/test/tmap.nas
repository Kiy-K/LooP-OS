...
;; ========================================================================
;;  Rasterization des segments d'un polyg\x93ne textur\x82 de mani\x8are LINEAIRE.
;;  Il s'agit donc d'interpoler les coordonn\x82es aux bords de la texture en
;;  m\x88me temps que les abscisses minx/maxx pour chaque ligne.
;;  L'argument 'dir' indique quels bords de la texture sont interpol\xe9s:
;;    0 : segments associ\x82s aux bord SUPERIEUR et INFERIEUR ( TY constant )
;;    1 : segments associ\x82s aux bord GAUCHE    et DROITE    ( TX constant )
;; ========================================================================
;;
...
;;rasterize_left:       ;; on rasterize un segment \x85 la GAUCHE du polyg\x93ne
...
.L_rleft_h_loop:
        ...
        jmp     .L_finished
;;
;; on rasterize un segment \x85 la DROITE du polyg\x93ne
;;
.L_rasterize_right:
        mov     ecx,ebx
        sub     ecx,eax
...
;;;
;       add ecx,[ystep]                 ;;pr\x82a1
;       adc ah,dh
;       add edx,[xstep]
;       adc al,ch
;       and eax,0x3f3f
;       mov bl,[esi+eax]                ;;pr\x82b1
;       mov dl,[ebx]                    ;;pr\x82c1
;
;       add ecx,[ystep]                 ;;a2
;        adc ah,dh
